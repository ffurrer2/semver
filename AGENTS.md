...
- Each project-defined subcommand (`compare`, `filter`, `format`, `next`, `sort`, `validate`, `version`) lives in its own file.
- Cobra's built-in `completion` command is enabled (not implemented in a dedicated file under `cmd/semver/`).
...
| `task upgrade`             | Upgrade major dependencies via gomajor + indirect dependencies via `go get -u all` |
